...
const nodeTypes = {
  question: (props: NodeProps<QuestionData>) => {
    const {
              ...
              {answer ? (
                <>
                  ...
                  <p className="text-gray-700 text-xs">
                    ...
                    Nacional de Gases de Efecto Invernadero publicado en 2019
                    por la Secretaría de Ambiente y Desarrollo Sustentable de la
                    Nación. Las vacas son la principal fuente de emisiones de
                    metano (<span className={`tag`}>CH4</span>
                    ), seguido de las emisiones de este mismo gas provenientes
  ...
  text2image: memo((props: NodeProps<{ prompt: string; img: string }>) => {
    const {
      id,
      data: { prompt, img },
    } = props;
  ...
  source: memo((props: NodeProps<SourceData>) => {
    const {
      data: { content, source },
    } = props;
  ...
  audio: memo((props: NodeProps<{ audio: string; transcript: string }>) => {
    const {
